          ...
          {store.cart.cartItems.map((item) => (
            <Box key={item.id} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', my: 1, padding: 1, borderRadius: 2 }}> 
              <Box sx={{ display: 'flex', alignItems: 'center', gap: -4 }}>
                {item.quantity ===  1 ? (<IconButton size="small" color="error" onClick={() => onRemoveItem({productId: item.product.product.id})}>
                  <DeleteIcon fontSize="small" />
                </IconButton>): (<IconButton size="small" onClick={() => onRemoveItem({productId: item.product.product.id})} disabled={item.quantity <= 1}>
                ...
                <IconButton size="small" onClick={() => onUpdateQuantity({productId: item.product.product.id})}>
